...
<?php
    if(isset($_POST)){
        ...
        if(array_key_exists('first_tube', $results)){
            ...
            $sql = "INSERT INTO tubes_tbl(id, job, setup_op, coil_change, coil, fil_mesh_change_top, filter_mesh_top, fil_mesh_change_bot, filter_mesh_bot, drain_change_top, drain_mesh_top, drain_change_bot, drain_mesh_bot, mill_time, mill_check) VALUES('$tube_id', '$job', '$setup_op', '".$result['new_coil']."', '$coil', '".$result['new_filter_top']."', '$filterTop', '".$result['new_filter_bot']."', '$filterBot', '".$result['new_drain_top']."', '$drainTop', '".$result['new_drain_bot']."', '$drainBot', '$timeStamp', 0)";
        ...
        if(array_key_exists('final_insp_geo', $results)){
            ...
            $sql = "UPDATE tubes_tbl SET id_drift = '$id_drift', od_check3 = '$od_check3', length_check2 = '$length_check2', weld = '$weld', repairs = '$repairs', ring_num1 = '$ring_num1', ring_num2 = '$ring_num2', inspector = '$inspector', insp_time = '$timeStamp', insp_check = 1 WHERE id = '$tube_id'";
        ...
        if(array_key_exists('insp_station_chksheet', $results)){
            ...
            $inspector = $result['inspector'];
            $tube_id = $result['tube_id'];
            // $drift_dim = $result['dim'];
            // $drift_insp = $result['dimInsp'];
            $job = $result['job'];
            print_r($result);
        ...
        if(array_key_exists('tack_weld_checksheet', $results)){
            $result= $results['tack_weld_checksheet'];

            date_default_timezone_set("US/Central");
            $tube_id_from = $result['tube_id_from'];
            $tube_id_to = $result['tube_id_to'];
            $tack_weld_insp = $result['tack_weld_insp'];
            $job = $result['job'];
            $timeStamp = date("Y-m-d H:i:s");
            ...
            $jobInfo = mysqli_fetch_array($resultConn);
            
            
            for($i = intval($tube_id_from); $i <= intval($tube_id_to); $i++){
                //convert tube number to tube id format 
                $tube_id = $job.'-'.str_pad(strval($i), strlen($jobInfo['quantity']),"0", STR_PAD_LEFT);
                $sql1 = "UPDATE tubes_tbl SET tack_weld = 1, tw_insp = '$tack_weld_insp', tw_time = '$timeStamp' WHERE id = '$tube_id'";
