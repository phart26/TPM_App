                ...
                <tbody>
                    <?php foreach($ordersACT as $order): ?>
                            <tr>
                                ...
                                <td style ="vertical-align: middle;" class="firstRow"><?php echo ($order['has_started'] == 1 || ($order['device'] != "" && $order['has_finished'] == 0)  ? "<a href='timeStamp.php?job=".$order['job']."&company=".getCustomer($order['cust_id'])."'>" . htmlspecialchars($order['job']) . "</a> " : htmlspecialchars($order['job'])); ?></td>
