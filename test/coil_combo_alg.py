...
def getCoilCombos(initCoilList, maxNumSplices, maxLen):
    ...
    while numCoils > 1:
        ...
        while len(combinedCoil) < maxNumSplices and sumCoils(combinedCoil, 'length') < maxLen and spot >= 0:

            # if value at spot key of initCoilList > 0 check if key plus sum of combo list is <= maxLen
            if initCoilList[spot]['quantity'] > 0:
                if (sumCoils(combinedCoil, 'length') + initCoilList[spot]['length']) <= maxLen:
...
def main():
    ...
    if(len(rv) > 0):
        ...
        # add on remaining coil if there
